// MAXCOOP Subscription Form - Enhanced Form Logic
// ================================================
// This file contains ALL form handling including new document uploads
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Store uploaded ID documents (existing)
    const idDocuments = {
        NATIONAL_ID: null,
    ...
    // Store OTHER uploaded documents (NEW)
    const otherDocuments = {
        // Individual/Proprietary
        INDIVIDUAL_INTERNATIONAL_PASSPORT: null,
        INDIVIDUAL_VOTER_ID: null,
        INDIVIDUAL_DRIVING_LICENSE: null,
        INDIVIDUAL_PASSPORT_PHOTOS: null,
        INDIVIDUAL_CERT_REGISTRATION: null,
        // Company
        COMPANY_CERT_INCORPORATION: null,
        COMPANY_MEMORANDUM: null,
        COMPANY_ARTICLES: null,
        COMPANY_VALID_ID: null,
        // Foundation
        FOUNDATION_CERT_REGISTRATION: null,
        FOUNDATION_TRUSTEES_PHOTOS: null,
        FOUNDATION_TRUSTEE_ID: null,
        // Partnership
        PARTNERSHIP_CERT_REGISTRATION: null,
        PARTNERSHIP_PARTNERS_PHOTOS: null,
        PARTNERSHIP_DEED: null,
        PARTNERSHIP_VALID_ID: null
    };
    ...
    // ========================================
    // OTHER DOCUMENTS UPLOAD TOGGLE (NEW)
    // ========================================
    window.toggleDocUpload = function(docType) {
        const uploadSection = document.getElementById(`upload_${docType}`);
        if (!uploadSection) return;

        const checkbox = document.querySelector(`input[name="otherDocs"][value="${docType}"]`);
        ...
        if (checkbox && checkbox.checked) {
            ...
            otherDocuments[docType] = null;
        }
    };

    // ========================================
    // FILE INPUT EVENT LISTENERS - ID DOCS
    // ========================================
    ...
    // ========================================
    // FILE INPUT EVENT LISTENERS - OTHER DOCS (NEW)
    // ========================================
    const otherFileInputs = Object.keys(otherDocuments);
    otherFileInputs.forEach(docType => {
        const fileInput = document.getElementById(`file_${docType}`);
        if (fileInput) {
            fileInput.addEventListener('change', function() {
                handleOtherDocUploadEvent(docType, this);
            });
        }
    });
    ...
    function handleOtherDocUploadEvent(docType, fileInput) {
        const preview = document.getElementById(`preview_${docType}`);
        const file = fileInput.files[0];
        ...
        const reader = new FileReader();
        reader.onload = function(e) {
            otherDocuments[docType] = {
                name: file.name,
                type: file.type,
                data: e.target.result
            };

            console.log('✅ Other Document Uploaded:', docType, file.name);
            ...
            if (file.type.startsWith('image/')) {
                preview.innerHTML = `
                    <div class="file-info">
                        <p>✓ ${file.name} uploaded</p>
                        <button type="button" class="remove-btn" onclick="removeOtherDocUpload('${docType}')">Remove</button>
                    </div>
    ...
    window.removeOtherDocUpload = function(docType) {
        const fileInput = document.getElementById(`file_${docType}`);
        const preview = document.getElementById(`preview_${docType}`);
        if (fileInput) fileInput.value = '';
        if (preview) {
    ...
    form.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            console.log('📦 Submitting with:', {
                ...
                otherDocs: Object.keys(otherDocuments).filter(k => otherDocuments[k]),
                officeLocation: formData.officeLocation,
                tipsRead: formData.tipsRead
            });

            // Generate PDF (function from pdf-generator.js)
            const pdfBlob = await generatePDF(formData, passportImageData, idDocuments, otherDocuments);
    ...
    function validateForm() {
        ...
        if (idCheckboxes.length > 0) {
            for (let checkbox of idCheckboxes) {
                ...
                if (!idDocuments[idType] && !otherDocuments[idType]) {
        ...
        // Check OTHER document uploads
        const otherDocCheckboxes = form.querySelectorAll('input[name="otherDocs"]:checked');
        if (otherDocCheckboxes.length > 0) {
            for (let checkbox of otherDocCheckboxes) {
                const docType = checkbox.value;
                if (!otherDocuments[docType]) {
                    alert(`⚠️ Please upload your ${docType.replace(/_/g, ' ')}`);
                    return false;
                }
            }
        }

        // Check Tips Read checkbox
        const tipsRead = document.getElementById('tipsRead');
        if (!tipsRead.checked) {
            alert('⚠️ Please confirm that you have read all TIPS TO NOTE ABOUT MAXCOOP');
            tipsRead.scrollIntoView({ behavior: 'smooth', block: 'center' });
            return false;
        }

        // Check Office Location selection
        const officeCheckboxes = form.querySelectorAll('input[name="officeLocation"]:checked');
        if (officeCheckboxes.length === 0) {
            alert('⚠️ Please select the office where your purchase was made');
            document.querySelector('.office-selection').scrollIntoView({ behavior: 'smooth', block: 'center' });
            return false;
        }
        ...
        for (let field of requiredFields) {
            if (field.type === 'radio') {
                ...
                if (field.name === 'tipsRead' || field.name === 'agreeToTerms') {
                    if (!field.checked) {
                        alert('⚠️ Please check all required checkboxes');
                        field.focus();
                        return false;
                    }
                } else {
    ...
    function collectFormData() {
        return {
            ...
            declaration: {
                ...
                buildingTimeline: getCheckedValue('buildingTimeline') || 'N/A',
            ...
            banking: {
                ...
                rcNumber: form.rcNumber.value || 'N/A',
                salaryRange: getCheckedValue('salaryRange') || 'N/A'
            },
            otherDocuments: {
                selected: getCheckedValues('otherDocs').join(', ') || 'None'
            },
            officeLocation: getCheckedValue('officeLocation') || 'Not Selected',
            tipsRead: document.getElementById('tipsRead').checked,
    ...
    setupSingleCheckbox('buildingTimeline');
    setupSingleCheckbox('salaryRange');
    setupSingleCheckbox('officeLocation'); // NEW
